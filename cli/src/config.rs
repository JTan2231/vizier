...
pub async fn llm_request(
    ...
    Ok(
        wire::prompt_with_tools(get_config().provider, &system_prompt, conversation, vec![])
            .await?,
    )
}
...
pub async fn llm_request_with_tools(
    ...
    Ok(wire::prompt_with_tools(get_config().provider, &system_prompt, conversation, tools).await?)
}
