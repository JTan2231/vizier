...
- Agent commands auto-discover when `agent.command` is empty: Vizier searches `PATH` for supported CLIs (preferring Codex, then Gemini) and builds a sensible default command per backend. For clean stdout (final assistant text only), point the command at a script shim; raw CLIs may emit JSON/events that you must filter in your script.
