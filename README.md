...
- Agent backends now run through shell scripts: stdout is the final assistant response, stderr carries progress/usage/errors, and the exit code sets status. Pick a bundled shim via `agent.label` (`codex`/`gemini`, installed under `share/vizier/agents/`) or point `[agents.<scope>.agent].command` at your own script. CLI overrides mirror the same levers: `--agent-label` or `--agent-command`.
