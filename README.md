...
- Install completions: `vizier completions --shell zsh >> ~/.zshrc` (or `source <(vizier completions --shell bash)` for Bash, `vizier completions --shell fish | source` for Fish) so Tab lists pending plan slugs for `vizier approve`/`vizier merge`
