...
Installed reference pages for this workflow are available via `man vizier`,
`man vizier-build`, `man vizier-jobs`, `man 5 vizier-config`, and
...
- Builds a merge commit with subject `feat: merge plan <slug>` and a body that only contains
  ...
  - An `Implementation Plan:` block that inlines the stored plan document (recovering the latest matching historical copy when the tip file has already been removed, or using a placeholder only when no valid copy exists).
