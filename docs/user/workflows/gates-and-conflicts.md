...
The init contract covers `.vizier/narrative/{snapshot,glossary}.md`, `.vizier/config.toml`, `.vizier/develop.hcl`, `.vizier/workflows/{draft,approve,merge,commit}.hcl`, root `ci.sh`, and required `.vizier/*` ignore rules.
