...
- List output: `[display.lists.*].format`/`fields` ↔ CLI `vizier list --format/--fields`, `vizier jobs list --format`, `vizier jobs show --format`.
- Display/help: pager defaults are TTY-only and honor `$VIZIER_PAGER`; `--no-ansi` strips color; `-q/-v/-vv` control verbosity.
...
- Per-command selector overrides: `[agents.commands.save|draft|approve|review|merge|patch|build_execute] agent="<selector>"`; CLI `--agent` overrides command/template selector tables for the current run.
...
- Workflow template compilation rejects undeclared artifact contracts, invalid artifact-contract schemas, schema-invalid artifact payloads, unknown `after` node references, and invalid capability contracts (approve/review/merge gate loops, gate cardinality, and schedulable capability argument shapes) before a job is queued.
...
- `vizier jobs schedule`: output is command-scoped (not config-driven) with `--format summary|dag|json`; default is human-readable `summary` (one row per visible job with deterministic `created_at` then `job_id` ordering), `--format dag` preserves recursive dependency traversal, and `--format json` emits a stable contract with `{version, ordering, jobs[], edges[]}` where `version=1` and `ordering="created_at_then_job_id"`.
...
- `vizier jobs status`: prints a terse one-line status; `--format json` emits `job`, `status`, `exit_code`, `stdout`, and `stderr` fields.
- `vizier jobs retry`: rewinds a failed/blocked job chain and prints a block outcome with `Requested`, `Retry root`, `Last successful point`, `Retry set`, `Reset`, `Restarted`, and optional `Updated`; `--format json` emits the same fields as structured arrays/strings.
- `vizier jobs approve` / `vizier jobs reject`: resolve explicit scheduler approval gates; `--format json` includes the resulting `status` and `approval_state`.
- CLI overrides: `vizier list --format`, `vizier list --fields` override the list display settings; `vizier jobs list --format`, `vizier jobs show --format`, `vizier jobs schedule --format`, `vizier jobs status --format`, `vizier jobs retry --format`, `vizier jobs approve --format`, and `vizier jobs reject --format` select command-local machine output.
...
- Help paging honors `$VIZIER_PAGER` (defaults to `less -FRSX`) and only pages on TTY by default.
- Progress is now purely line-based: `-v`/`-vv` lift stderr verbosity while machine-readable output is command-local (`vizier plan --json`, `--format json` on JSON-capable commands).
