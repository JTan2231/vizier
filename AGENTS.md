...
Process-backed commands now resolve to a concrete `AgentRunner` + `AgentDisplayAdapter` pair per scope. Codex implements both pieces today: the runner shells out to `codex exec` while the display adapter turns Codex's JSON events into Vizier's `[agent:<scope>] phase — message` history lines. The config resolver attaches those objects once per command (`[agents.ask]`, `[agents.merge]`, etc.), so future CLI agents just plug in their own runner/adapter without touching `draft → approve → review → merge`. When a scope opts into `backend = "wire"`, Vizier swaps in a fallback adapter so wire events still render coherently even though no process runner is present.
