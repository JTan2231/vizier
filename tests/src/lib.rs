...
impl IntegrationRepo {
    ...
    fn vizier_cmd_base(&self) -> Command {
    ...
    fn vizier_cmd_background(&self) -> Command {
...
fn wait_for_job_completion(repo: &IntegrationRepo, job_id: &str, timeout: Duration) -> TestResult {
    let job_path = repo
        .path()
        .join(".vizier/jobs")
        .join(job_id)
...
fn wait_for_job_active(repo: &IntegrationRepo, job_id: &str, timeout: Duration) -> TestResult {
    let job_path = repo
        .path()
        .join(".vizier/jobs")
        .join(job_id)
...
fn list_merge_job_ids(repo: &IntegrationRepo) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let jobs_root = repo.path().join(".vizier/jobs");
    if !jobs_root.exists() {
        return Ok(Vec::new());
    }
...
fn read_merge_queue_state(repo: &IntegrationRepo) -> Result<Value, Box<dyn std::error::Error>> {
