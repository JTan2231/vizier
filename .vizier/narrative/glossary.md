...
- **Plan inventory drift**: Divergence between `.vizier/implementation-plans/*.md` artifacts and local `draft/<slug>` branches (for example, current docs are `refactor.md` + `removing-wire.md` while `.vizier/implementation-plans/release.md` is pending deletion and local draft branches include `draft/after`, `draft/approve`, `draft/init`, and `draft/release`), which makes pending-plan surfaces (`vizier list`, completions, plan status) less trustworthy until reconciled.
