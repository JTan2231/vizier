...
- **Plan inventory drift**: Divergence between `.vizier/implementation-plans/*.md` artifacts and local `draft/<slug>` branches (for example, current docs are `refactor.md` + `removing-wire.md` while local draft branches include `draft/after`, `draft/approve`, `draft/init`, and `draft/reduction`, with `build/patch-740958c5f8bf` also present outside the draft mapping), which makes pending-plan surfaces (`vizier list`, completions, plan status) less trustworthy until reconciled.
