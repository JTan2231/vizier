...
- **Merge conflict gate index finalization**: `gate.conflict_resolution` behavior that, after a successful auto-resolve pass, stages only the originally conflicted paths via allow-missing semantics, rechecks the unmerged index, clears the merge sentinel only when no unmerged entries remain, and otherwise blocks with explicit remaining-path diagnostics.
...
- **Run batch check summary**: Additive `vizier run --spec-dir ... --check` validation output that reports discovered batch items (`index`, `spec_file`, `slug`) without writing manifests/jobs or ticking the scheduler.
...
- **Run spec-dir batch expansion**: `vizier run --spec-dir <dir>` queue-time batch mode for workflows that declare `spec_file`; it recursively discovers repo-local `*.md` specs, supplies each item's `spec_file` and derived `slug` from the batch-root relative stem, validates every expanded item before enqueue, serializes expanded runs through prior-run success sinks, and rejects explicit `spec_file`, `slug`, or shared `branch` overrides because those values are batch-owned.
...
- **Run batch aggregate payload**: Multi-run JSON/text summary shape used by `vizier run --spec-dir` (`workflow_runs_enqueued` / `workflow_runs_terminal`, `batch_dir`, `spec_count`, and ordered per-run `spec_file` + `slug` records).
