...
- **Plan inventory drift**: Divergence between `.vizier/implementation-plans/*.md` artifacts and local `draft/<slug>` branches (for example, plan docs currently centered on `refactor.md`/`removing-wire.md`, while recently tracked docs like `display.md` and `patch-740958c5f8bf-03-bug3-merge.md` are deleted/pending deletion across worktrees and observed branch sets include `draft/after`, `draft/approve`, `draft/approve-1`, `draft/display`, `draft/init`, `draft/patch-*`, and `draft/true-patch`), which makes pending-plan surfaces (`vizier list`, completions, plan status) less trustworthy until reconciled.
