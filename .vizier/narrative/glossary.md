...
- **Run spec-dir batch expansion**: `vizier run --spec-dir <dir>` queue-time behavior that recursively discovers repo-local `*.md` specs, derives per-item slugs from batch-root relative stems, validates every item before enqueue, and serializes expanded runs through prior-run success sinks.
