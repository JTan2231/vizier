...
- **Run flow resolution order**: `FLOW` lookup contract: explicit `file:`/path, then `[commands]` alias, then selector lookup, then repo fallback files (`.vizier/<flow>.{toml,json}`, `.vizier/workflows/<flow>.{toml,json}`, legacy `.vizier/workflow/<flow>.{toml,json}`), then global implicit alias files (`<global_workflow_dir>/<flow>.{toml,json}`) when enabled.
