...
- **Plan inventory drift**: Divergence between `.vizier/implementation-plans/*.md` artifacts and local `draft/<slug>` branches (for example, docs `refactor.md`/`removing-wire.md` while branches include `draft/approve`, `draft/display`, and `draft/patch-*`, with tracked `display.md` deleted locally), which makes pending-plan surfaces (`vizier list`, completions, plan status) less trustworthy until reconciled.
