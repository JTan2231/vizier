...
- **Plan inventory drift**: Divergence between `.vizier/implementation-plans/*.md` artifacts and local `draft/<slug>` branches (for example, plan docs still at `refactor.md`/`removing-wire.md` plus a pending delete of `patch-740958c5f8bf-03-bug3-merge.md`, while branches include `draft/after`, `draft/approve`, `draft/approve-1`, `draft/init`, `draft/patch-740958c5f8bf-03-bug3-merge`, `draft/patch-740958c5f8bf-04-bug4-retry`, and `draft/true-patch`), which makes pending-plan surfaces (`vizier list`, completions, plan status) less trustworthy until reconciled.
