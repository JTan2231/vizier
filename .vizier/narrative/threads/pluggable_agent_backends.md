...
Update (2025-11-24, follow-up): Progress filters own the final reply
- ScriptRunner now prefers stdout from the configured progress filter as the assistant output, so JSON-parsing filters can render progress without losing the final message; filter stdout is no longer dropped or misfiled into stderr when a filter is present.
