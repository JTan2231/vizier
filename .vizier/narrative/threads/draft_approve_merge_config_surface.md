...
Update (2026-02-14): Decomposed `docs/user/workflows/draft-approve-merge.md` into a stable landing/router plus focused stage docs (`alias-run-flow`, `stage-execution`, `gates-and-conflicts`) with a frozen source→target map, keeping command semantics unchanged while reducing configuration-surface cognitive load for operators.
