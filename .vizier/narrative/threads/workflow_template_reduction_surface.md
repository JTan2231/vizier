...
- Update (2026-04-04, merge conflict gate index finalization):
  - `gate.conflict_resolution` now captures the original unmerged path set before any auto-resolve action, stages only that set via `stage_paths_allow_missing_in(...)` after a successful resolver pass, and rechecks the index before deciding whether to clear the merge sentinel.
  - Successful auto-resolve now covers both content rewrites and conflict-file deletions without requiring resolver-side `git add`, while unrelated dirty files remain unstaged because the runtime never widens the staging scope beyond the original conflict set.
  - Blocked merge recovery now reports `merge conflict resolution incomplete ...: unmerged index entries remain` and includes the remaining unmerged path list in stderr, making the “resolved worktree, unresolved index” failure mode visible to operators.
  - Runtime and integration coverage now assert edit-without-stage, delete-without-stage, lingering-unmerged diagnostics, unrelated-dirty isolation, and composed `develop` auto-resolve recovery without sequential-run wedging.
