...
Update (2026-02-07): Hardened gate determinism under full-suite load by tightening the integration harness: `IntegrationRepo` fixtures now serialize via a poison-tolerant global lock, each fixture executes a local copy of the cached `vizier` binary, and the `jobs tail --follow` ordering assertion now validates per-stream chronology instead of brittle cross-stream interleaving. These changes keep `./cicd.sh` failures scoped to real regressions instead of harness-level `ENOENT`/ordering flakes.
Update (2026-02-08): Fixed a gate-blocking compile regression in `tests/src/patch.rs`: helper calls (`run_patch_follow`, job-record loading/scope filters) no longer pointed at missing local functions, and an explicit-pipeline assertion now reads `output.stdout` instead of a stale `first.stdout` binding. `./cicd.sh` reran green after the repair.
