...
- Documentation path guidance is still split: AGENTS.md/README use root shorthand paths while canonical docs live under `docs/user/*`; root alias shims now resolve dead links, but we still risk alias/canonical drift unless both surfaces stay synchronized.
...
Update (2026-02-14): Docs-path dead-link risk was reduced by adding root compatibility aliases (`docs/config-reference.md`, `docs/prompt-config-matrix.md`) that point to canonical `docs/user/*` pages, and workflow guidance was decomposed so `docs/user/workflows/draft-approve-merge.md` now routes to focused pages (`alias-run-flow`, `stage-execution`, `gates-and-conflicts`). Remaining work is governance on whether AGENTS/README should keep alias paths or switch to direct canonical links, plus keeping alias shims synchronized.
Update (2026-02-14, pager flag contract drift): Re-validated the help pager surface in this worktree: help remains TTY-auto with `$VIZIER_PAGER` and hidden `--no-pager`, but explicit `--pager` is rejected as an unexpected argument (`vizier help --pager`, `vizier list --pager`). AGENTS.md currently advertises `--pager` as user-facing, so docs/CLI contract reconciliation remains active.
Update (2026-02-15, canonical-only config hard cut): Config/runtime compatibility bridges were removed. Loader/validator now hard-fail legacy `[workflow.templates]` and `[agents.<scope>]`, dotted selectors (`template.name.vN`) fail with migration guidance to `template.name@vN`, and `vizier run <flow>` no longer performs implicit repo/global flow-name fallback discovery.
Update (2026-02-20, pager contract closure): Reconciled operator guidance with runtime behavior: config/workflow docs now explicitly state TTY-only auto-paging and non-TTY direct help output, no user-facing `--pager` flag is documented, and integration tests now lock `vizier help --pager` / `vizier list --pager` to generic unknown-argument failures without custom migration guidance.
