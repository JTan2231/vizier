...
- Repo-local stage templates now ship as canonical primitive DAGs (`template.stage.{draft,approve,merge}@v2`) under `.vizier/workflow/*.toml`:
  ...
  - merge: `git.integrate_plan_branch` plus canonical conflict/CI gates (`gate.conflict_resolution`, `gate.cicd`) routed before `terminal`.
...
- `vizier run draft|approve|merge` executes canonical primitive stage DAGs from `.vizier/workflow/*.toml` (no `vizier.*` `uses` labels) with expected stage artifacts/gates.
