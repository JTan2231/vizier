# Chat path: Auditor integration + commit gate cohesion (Thread: Commit isolation + gates)

Problem/Tension
- Actions executed through the chat path bypass the Auditor’s accountability trail and the automatic commit mechanism of `ask`. This breaks the promise that AI-led changes are auditable and gated.
...
- Every tool/action initiated during chat is funneled through the Auditor with a uniform audit record (who/what/why/when) and participates in the Pending Commit gate, identical to `ask`.
- When auto_commit=false or confirm_destructive=true, changes pause at the Pending Commit gate with a CLI-visible affordance to accept/reject; acceptance produces a conventional commit authored as “vizier-assistant” with the chat context captured in the commit body. Rich UI controls remain future work.
...
Acceptance Criteria
1) Auditor Path: Triggering a code-modifying action from chat produces an Auditor entry and a corresponding pending change set that is visible to the VCS layer without committing by default.
2) Gate Parity: With auto_commit=false, after an action the CLI surfaces a Pending Commit banner/epilogue with options [Accept Commit], [View Diff], [Discard]; selecting Accept creates a commit; Discard rolls back all staged changes from that action. Future UI surfaces can layer richer controls.
...
Pointers
- Surfaces: vizier-core::{auditor.rs, vcs.rs, chat.rs}; vizier-cli/src/actions.rs; renderer-neutral events for future UI surfaces.
